...
@irdl_op_definition
class CallIndirect(IRDLOperation):
    ...
    @classmethod
    def parse(cls, parser: Parser) -> CallIndirect:
        callee, arguments, results, extra_attributes = parse_call_op_like(
            parser, reserved_attr_names=("callee",)
        )
        call = CallIndirect(callee, arguments, results)
        if extra_attributes is not None:
            call.attributes |= extra_attributes.data
        return call
...
class Constant(IRDLOperation):
    name = "func.constant"
    value: FlatSymbolRefAttr = prop_def(FlatSymbolRefAttr)
    # Note: naming this results triggers an ArgumentError
    ...
    def __init__(
        self,
        value: str | SymbolRefAttr,
        return_types: Sequence[Attribute],
    ):
        if isinstance(value, str):
            value = SymbolRefAttr(value)
        super().__init__(
            operands=[],
            result_types=return_types,
...
Func = Dialect("func", [FuncOp, Call, CallIndirect, Constant, Return], [])
