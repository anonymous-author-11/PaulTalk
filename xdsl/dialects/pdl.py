...
class ReplaceOp(IRDLOperation):
    ...
    * a set of `Value`s (`replValues` should be populated)
    ...
    repl_values: VarOperand = var_operand_def(
        base(ValueType) | base(ArrayAttr[ValueType])
    )
...
class ResultsOp(IRDLOperation):
    ...
    parent_: Operand = operand_def(OperationType)
    val: OpResult = result_def(base(ValueType) | base(RangeType[ValueType]))
...
class FuncOp(IRDLOperation):
    ...
    function_type: StringAttr = prop_def(StringAttr)
...
PDLINTERP = Dialect("pdl_interp", [FuncOp, FinalizeOp], [])