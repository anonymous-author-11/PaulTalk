...
class CompilationJob:
    ...
    def run_lto(self):
        ...
        make_archive = (LLVM_AR_PATH, "cr", out2_path, *thin_paths)
        run_checked(make_archive)

        if self.settings.debug_mode:            
            with open(self.build.out_linked, "r") as f: linked_ir = f.read()
            linked_ir = remove_invariant_on_globals(linked_ir)
            linked_ir = run_checked((LLVM_LINK_PATH, "-S", "-", out2_path, "--only-needed"), input_text=linked_ir)
            self.write_side_ir(self.build.dir / "after_lto.ll", linked_ir)
            if self.output_path and self.output_path.suffix == ".exe":
                linked_ir = remove_fluff(linked_ir, [self.build.dir / "after_lto.ll"])
            self.write_side_ir(self.build.out_optimized, linked_ir)
        ...
        full_lto = (
            ...
            out2_path,
            "--lto=full", passes, "--lto-emit-llvm",
            "-o", lto_file, *self.settings.lto_options
        )
        ...
        # Remove a hell of a lot of binary size
        if self.output_path and self.output_path.suffix == ".exe":
            optimized_ir = remove_fluff(optimized_ir, [self.build.dir / "after_lto.ll"])
    ...
    def run_lld(self):
        ...
        includes = (*dynamic_libc, "Dbghelp.lib", "gc.lib")
...
class OptimizationSettings:
    ...
    def devirt(self):
        ...
        return "--max-devirt-iterations=200 --abort-on-max-devirt-iterations-reached"
